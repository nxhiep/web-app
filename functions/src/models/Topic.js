...
class Topic {
  constructor() {
    ...
    this.decks = decks !== null && decks !== void 0 ? decks : []; // console.log("2222222222222222222222222222222222", id, progress);

    if (progress) {
      this.progress = _TopicProgress.default.fromJS(progress);
    } else {
      this.progress = _TopicProgress.default.init(this.id, _config.default.USER_ID);
    }
  }

  static getArguments() {
    return ['instanceFeedback', 'totalQuestion', 'contentTest', 'topicId', 'id', 'parentId', 'status', 'rootTopicId', 'shortId', 'name', 'description', 'orderIndex', 'allowMistakes', 'lastUpdate', 'childrentType', 'level', 'cardShortIds', 'decks', 'progress'];
  }
  ...
  static fromJS(topicEntity) {
    ...
    if (typeof topicEntity === 'string') {
      ...
      var convert = {};
      var keysOfChoiceJS = Object.keys(topicEntity);
      var valuesOfChoiceJS = Object.values(topicEntity);
      ...
      var choice = Object.create(Topic.prototype);
  ...
  getPercentComplete() {
    if (this.progress) {
      ...
      var p = (this.progress.familiar * 0.5 + this.progress.mastered * 1) / this.totalQuestion;
