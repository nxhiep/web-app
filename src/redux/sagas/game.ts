...
import Question from '../../models/QuestionX.js';
...
function* startGame() {
    ...
    let appId = action.appId;
    let gameType = action.gameType;
    let testSetting: TestSetting;
    if (action.setting) {
        // console.log("vllll", setting);
        testSetting =  TestSetting.fromJS(action.setting);
    } else {
        testSetting = new TestSetting(yield select((state: AppState) => {
    ...
    if (gameType === Config.STUDY_GAME || gameType === Config.REVIEW_GAME) {
        ...
        currentGame = yield select((state: AppState) => {
            let games: Array<GameState> = state.listGameState.games;
            return games.find((item: GameState) => (item.id == topicId && item.gameType == gameType));
        });
        ...
        currentGame = yield select((state: AppState) => {
            let games: Array<GameState> = state.listGameState.games;
            return games.find((item: GameState) => (item.id == topicId && item.gameType == Config.TEST_GAME));
        });
    }
...
function* onSaveGame() {
    while (true) {
        try {
            ...
            let gameState = yield select((state: AppState) => state.gameState);
    ...
    while (true) {
        try {
            ...
            if(gameReducer.gameType != Config.TEST_GAME){
                let currentTopic: Topic = topicState.data[gameReducer.topicId];
                let selectedChoice: Choice = action.payload;
                let currentQuestion: Question = gameReducer.questions.find((item: Question) => {
                ...
                if(currentTopic && gameReducer.gameType == Config.STUDY_GAME){
                    ...
                    let currentParentTopic: Topic = topicState.data[currentTopic.parentId];
